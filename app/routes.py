...
def process_document_for_embedding(temp_path, document_id):
    try:
        raw_text = FileProcessor.get_file_text(temp_path)
        if not raw_text.strip():
            app.logger.error(f"Empty content in document {document_id}")
            return

        text_chunks = FileProcessor.get_text_chunks(raw_text)
        if not text_chunks:
            app.logger.error(f"No valid text chunks created for document {document_id}")
            return
            
        vector_store_manager.create_vector_store(text_chunks, document_id)
        ...
        # You might want to add error handling for specific cases
...
def delete_document():
    ...
    try:
        ...
        # Clean up any conversations related to this document
        global conversations
        conversations = {k: v for k, v in conversations.items() 
                        if not any(msg.get("document_id") == document_id for msg in v)}
